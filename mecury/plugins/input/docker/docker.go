...
func (d *Docker) Gather(acc agent.Accumulator) error {
	if d.client == nil {
		var c *client.Client
		var err error
...
func (d *Docker) gatherInfo(acc agent.Accumulator) error {
	// Init vars
...
func init() {
	agent.AddInput("docker", &Docker{
		PerDevice: true,
		Timeout:   misc.Duration{Duration: time.Second * 5},
	},
	)
}
