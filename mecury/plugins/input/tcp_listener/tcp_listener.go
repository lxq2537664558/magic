...
// function.
func (t *TcpListener) Gather(_ agent.Accumulator) error {
	return nil
}

func (t *TcpListener) SetParser(parser agent.Parser) {
	t.parser = parser
}

// Start starts the tcp listener service.
func (t *TcpListener) Start(acc agent.Accumulator) error {
	t.Lock()
	defer t.Unlock()
...
func init() {
	agent.AddInput("tcp_listener", &TcpListener{})
}
