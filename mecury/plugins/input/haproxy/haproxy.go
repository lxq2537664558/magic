...
func (g *haproxy) Gather(acc agent.Accumulator) error {
	if len(g.Servers) == 0 {
...
func (g *haproxy) gatherServerSocket(addr string, acc agent.Accumulator) error {
...
func (g *haproxy) gatherServer(addr string, acc agent.Accumulator) error {
...
func importCsvResult(r io.Reader, acc agent.Accumulator, host string) error {
	csv := csv.NewReader(r)
...
func init() {
	agent.AddInput("haproxy", &haproxy{})
}
