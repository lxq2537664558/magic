...
func (z *Zookeeper) Gather(acc agent.Accumulator) error {
	if len(z.Servers) == 0 {
		return nil
	}
...
func (z *Zookeeper) gatherServer(address string, acc agent.Accumulator) error {
...
func init() {
	agent.AddInput("zookeeper", &Zookeeper{})
}
