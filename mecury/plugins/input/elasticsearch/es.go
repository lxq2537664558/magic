...
// Accumulator.
func (e *Elasticsearch) Gather(acc agent.Accumulator) error {
	if e.client == nil {
	...
	for _, serv := range e.Servers {
		go func(s string, acc agent.Accumulator) {
			defer wg.Done()
			var url string
			if e.Local {
...
func (e *Elasticsearch) createHttpClient() (*http.Client, error) {
	tlsCfg, err := misc.GetTLSConfig(e.SSLCert, e.SSLKey, e.SSLCA, e.InsecureSkipVerify)
	if err != nil {
		return nil, err
	}
	tr := &http.Transport{
...
func (e *Elasticsearch) gatherNodeStats(url string, acc agent.Accumulator) error {
	nodeStats := &struct {
		ClusterName string           `json:"cluster_name"`
...
func (e *Elasticsearch) gatherClusterStats(url string, acc agent.Accumulator) error {
...
func init() {
	agent.AddInput("elasticsearch", NewElasticsearch())
}
